...
#Figure 2. The Distribution of the z-scored Unigram Values by Place, Sex and Age
Unigram_data <- mutate(Unigram_data, Age_Group = as.factor(ifelse(Unigram_data$Age <= 40,"25-40 (corrected: under 40)","45+")))
...
#Figure 3. The Distribution of the z-scored Delta Values by Place, Sex and Age
...
#Data_counted$X1prop <- Data_counted$"-1"/(Data_counted$"0" + Data_counted$"1" + 1)
...
Data_counted$Speaker_ID <- droplevels(Data_counted$Speaker_ID)
Data_counted$Place <- droplevels(Data_counted$Place)
